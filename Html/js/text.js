 ...
 * 		//response_str
 */
function _askTheFakeBot(city_str, input_str,creativity_str, cb){
	var 
		hard_coded_temp_int = 20.88,
		response_str = "";
	cb(g_customizeResponse(city_str, input_str, creativity_str, hard_coded_temp_int));
}
 ...
 * 		//response_str
 */
function _askTheAPIBot(city_str, input_str, creativity_str, cb){
	console.log("We are hitting the API: " + G_API_GATEWAY_URL_STR);
	console.log("Model String: " + city_str);
	var 
		params = {
			"first_name":input_str,
			"name_creativity": parseInt(creativity_str)/100,
			"model_name":city_str
			
		};
	console.log("Params: " + params.first_name);
	console.log("Params: " + params.name_creativity);
	console.log("Params: " + params.model_name);

	g_ajaxer(G_API_GATEWAY_URL_STR, params, function(response){
		handleGatewayResponse(response, city_str, input_str,creativity_str, cb);
	}, function(error){
		handleGatewayError(error, cb);
	});
}

function handleGatewayResponse(response, city_str,input_str,creativity_str, cb){
	var 
		babynames_str = response.output_names;
	
	console.log("Babynames: " + babynames_str);
	console.log("Babynames: " + babynames_str.toString());
	
	cb(g_customizeResponse(city_str, input_str, creativity_str, babynames_str.toString()));
}
 ...
 */
function _pickABot(city_str,input_str,creativity_str, cb){
	if(city_str === ""){
		response_str = "Please pick a model type, thanks";
		return cb(response_str);
	}
	if(G_API_GATEWAY_URL_STR === null){
		_askTheFakeBot(city_str, input_str,creativity_str, cb);
	}else{
		_askTheAPIBot(city_str,input_str,creativity_str, cb);
	}
}
/**
 * whatShouldMyPetDo
...
function whatShouldMyPetDo(se){
	...
	input_str = $input.val();
	creativity_str =$myRange.val();
	_pickABot(city_str,input_str, creativity_str, function(response_str){
